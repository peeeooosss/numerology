  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        {showPreferredTime && (
          <div>
            <Label htmlFor={`${idPrefix}-time`}>Preferred time (IST, optional)</Label>
      ...
      <div>
        <Label htmlFor={`${idPrefix}-focus`}>Primary focus area</Label>
          <select id={`${idPrefix}-focus`} value={form.focusArea} onChange={(event) => updateFocus(event.target.value)} className="min-h-12 w-full rounded-xl border border-white/10 bg-[#0d0f20] px-3 py-2 text-base text-cream focus:border-gold/40 focus:outline-none focus:ring-2 focus:ring-gold/20">
      ...
      <div>
        <Label htmlFor={`${idPrefix}-subfocus`}>What would you like to focus on?</Label>
          <select id={`${idPrefix}-subfocus`} required value={form.subFocusArea} onChange={(event) => updateField("subFocusArea", event.target.value)} className="min-h-12 w-full rounded-xl border border-white/10 bg-[#0d0f20] px-3 py-2 text-base text-cream focus:border-gold/40 focus:outline-none focus:ring-2 focus:ring-gold/20">
      ...
      <div>
        <Label htmlFor={`${idPrefix}-question`}>Main question</Label>
          <textarea id={`${idPrefix}-question`} required rows={3} value={form.question} onChange={(event) => updateField("question", event.target.value)} placeholder="What would you like to discuss with the Master?" className="min-h-24 w-full rounded-xl border border-white/10 bg-[#0d0f20] px-3 py-3 text-base text-cream placeholder:text-lav/70 focus:border-gold/40 focus:outline-none focus:ring-2 focus:ring-gold/20" />
      </div>

      <div>
        <Label htmlFor={`${idPrefix}-outcome`}>Desired outcome (optional)</Label>
          <textarea id={`${idPrefix}-outcome`} rows={2} value={form.desiredOutcome} onChange={(event) => updateField("desiredOutcome", event.target.value)} placeholder="What clarity or decision would make this session useful?" className="min-h-20 w-full rounded-xl border border-white/10 bg-[#0d0f20] px-3 py-3 text-base text-cream placeholder:text-lav/70 focus:border-gold/40 focus:outline-none focus:ring-2 focus:ring-gold/20" />
      </div>

      <div>
        <Label htmlFor={`${idPrefix}-context`}>Additional context (optional)</Label>
          <textarea id={`${idPrefix}-context`} rows={2} value={form.additionalContext} onChange={(event) => updateField("additionalContext", event.target.value)} placeholder="Share any dates, choices, or background that would help the Master prepare." className="min-h-20 w-full rounded-xl border border-white/10 bg-[#0d0f20] px-3 py-3 text-base text-cream placeholder:text-lav/70 focus:border-gold/40 focus:outline-none focus:ring-2 focus:ring-gold/20" />
      </div>
