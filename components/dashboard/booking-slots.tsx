...
const demoIntake: SessionIntakeValues = { ...initialSessionIntake };
...
export function BookingSlots({ onBooked, serviceType = "numerology" }: Props) {
  ...
  async function submitBooking(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
       const orderResponse = await fetch("/api/payments/create-order", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ productType: serviceType, name: form.name, email: form.email }) });
       ...
       if (order.mode !== "development") {
         const payment: RazorpayPaymentResponse = await openRazorpayCheckout({ key: order.keyId, amount: order.amount, currency: order.currency, orderId: order.orderId, name: "Magic of Numbers", description: service.name, prefill: { name: form.name, email: form.email, contact: form.phone } });
        ...
        <div className="modal-safe-padding fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-midnight/85 backdrop-blur-sm sm:items-center" role="dialog" aria-modal="true" aria-labelledby="dashboard-booking-title">
          <div className="relative my-auto max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto overscroll-contain rounded-2xl border border-gold/30 bg-[#14162d] p-5 shadow-cardglow sm:p-6 md:max-h-[90vh] md:p-8">
            ...
            {error && <p role="alert" className="mt-4 rounded-lg border border-rose-400/30 bg-rose-400/10 p-3 text-sm text-rose-200">{error}</p>}
              {processing ? <div className="py-14 text-center"><Loader2 className="mx-auto h-10 w-10 animate-spin text-gold" /><p className="mt-4 text-sm text-lav">Opening secure payment and reserving this time…</p></div> : <form className="mt-6" onSubmit={submitBooking}>{serviceType === "name-balancing" ? <NameBalancingIntakeFields form={form} updateField={updateField} idPrefix="dashboard-name-balancing" showPreferredTime={false} /> : <SessionIntakeFields form={form} updateField={updateField} idPrefix="dashboard-session" showPreferredTime={false} />}<div className="mt-4 rounded-xl border border-gold/15 bg-gold/5 p-3"><div className="flex items-center gap-2 text-sm text-gold"><LockKeyhole className="h-4 w-4" /><span>Secure Razorpay payment · ₹{service.price} one-time</span></div></div><Button type="submit" className="mt-4 w-full">Pay and book · ₹{service.price} <CalendarCheck className="h-4 w-4" /></Button></form>}
          </div>
        </div>
      )}
    </section>
