...
export function BookingSlots({ onBooked, serviceType = "numerology" }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-gold/15 bg-white/[.025] p-5 shadow-cardglow md:p-6">
      ...
      {confirmed && <div className="mt-5 rounded-xl border border-emerald-400/25 bg-emerald-400/10 p-4"><div className="flex items-center gap-2 text-emerald-200"><CheckCircle2 className="h-4 w-4" />Session booked successfully</div><p className="mt-2 text-sm text-cream">{confirmed.time}</p><p className="mt-1 font-mono text-[11px] text-lav">Booking ID: {confirmed.id}</p></div>}
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-midnight/85 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="dashboard-booking-title">
          <div className="relative max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-gold/30 bg-[#14162d] p-6 shadow-cardglow md:p-8">
            <button type="button" onClick={() => setSelected(null)} aria-label="Close booking form" className="absolute right-4 top-4 rounded-lg p-2 text-lav hover:bg-white/10 hover:text-cream"><X className="h-4 w-4" /></button>
